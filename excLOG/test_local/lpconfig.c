				...
				if (sscanf(tmp,"%s",key)>0){
					...
					if (pos2-pos1>=0){
						...
						if (cur!=NULL){
							...
							fprintf(stderr,"found key,item but no sections");
						}
					}
				}
			}
...
*/
const char *lp_config_get_string_eX(LpConfig *lpconfig, const char *section, const char *key,char **pp_buf,size_t *size, const char *default_string){
	LpSection *sec;
	LpItem *item;
	GList *elem;
	char *p1 = NULL;
...
int lp_config_sync(LpConfig *lpconfig){
	...
	if (file==NULL){
		fprintf(stderr,"Could not write %s !",lpconfig->filename);
		return -1;
	}
