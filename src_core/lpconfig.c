...
void lp_item_destroy0(LpItem *item,void *udptr)
{
    free(item->key);
    free(item->value);
    free(item);
...
void lp_section_destroy0(LpSection *sec,void *udptr)
{
    free(sec->name);
    g_list_foreach(sec->items,(GFunc)lp_item_destroy0,NULL);
    g_list_free(sec->items);
    free(sec);
}

void lp_config_flush(LpConfig *lpconfig)
{
    g_list_foreach(lpconfig->sections,(GFunc)lp_section_destroy0,NULL);
    g_list_free(lpconfig->sections);
...
int lp_config_get_key_array(LpConfig *lpconfig,const char *section, int keyc, char *keyv[])
{
    int knum=0;
    LpSection *sec;
    LpItem *item;
