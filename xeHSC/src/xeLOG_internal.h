 ...
 **/
extern int _xeHSC_log_level;

#ifndef LOG_PREFIX
#define LOG_PREFIX "libxeHSC"
#endif

#if defined(CONFIG_XEHSC_DEVPLAT_LINUXGCC_32)
#define xeLOG_EMERG(fmt, args...) \
		do{ \
			if(_xeHSC_log_level>=(LOG_EMERG)) { \
				fprintf(stderr, LOG_PREFIX"(%s.%d.EMERG): "fmt"\n",__FILE__,__LINE__, ##args); \
				fflush(stderr); \
			} \
		}while(0);
...
#define xeLOG_CRIT(fmt, args...) \
		do{ \
			if(_xeHSC_log_level>=(LOG_CRIT)) { \
				fprintf(stderr, LOG_PREFIX"(%s.%d.CRIT): "fmt"\n",__FILE__,__LINE__, ##args); \
				fflush(stderr); \
			} \
		}while(0);
...
#define xeLOG_ALERT(fmt, args...) \
		do{ \
			if(_xeHSC_log_level>=(LOG_ALERT)) { \
				fprintf(stderr, LOG_PREFIX"(%s.%d.ALERT): "fmt"\n",__FILE__,__LINE__, ##args); \
				fflush(stderr); \
			} \
		}while(0);
...
#define xeLOG_ERR(fmt, args...) \
		do{ \
			if(_xeHSC_log_level>=(LOG_ERR)) { \
				fprintf(stderr, LOG_PREFIX"(%s.%d.ERR): "fmt"\n",__FILE__,__LINE__, ##args); \
				fflush(stderr); \
			} \
		}while(0);
...
#define xeLOG_WARNING(fmt, args...) \
		do{ \
			if(_xeHSC_log_level>=(LOG_WARNING)) { \
				fprintf(stderr, LOG_PREFIX"(%s.%d.WARNING): "fmt"\n",__FILE__,__LINE__, ##args); \
				fflush(stderr); \
			} \
		}while(0);
...
#define xeLOG_NOTICE(fmt, args...) \
		do{ \
			if(_xeHSC_log_level>=(LOG_NOTICE)) { \
				fprintf(stderr, LOG_PREFIX"(%s.%d.NOTICE): "fmt"\n",__FILE__,__LINE__, ##args); \
				fflush(stderr); \
			} \
		}while(0);
...
#define xeLOG_INFO(fmt, args...) \
		do{ \
			if(_xeHSC_log_level>=(LOG_INFO)) { \
				fprintf(stdout, LOG_PREFIX"(%s.%d.INFO): "fmt"\n",__FILE__,__LINE__, ##args); \
				fflush(stdout); \
			} \
		}while(0);
...
#define xeLOG_DEBUG(fmt, args...) \
		do{ \
			if(_xeHSC_log_level>=(LOG_DEBUG)) { \
				fprintf(stdout, LOG_PREFIX"(%s.%d.DBG): "fmt"\n",__FILE__,__LINE__, ##args); \
				fflush(stdout); \
			} \
		}while(0);
